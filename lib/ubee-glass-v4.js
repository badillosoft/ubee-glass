async function ubee_glass(name) {
            ...
            <script>
                ...
                for (let [variant, value] of Object.entries(self.dataset || {})) {
                    ...
                    if (variant.match(/([^-]+)-([\\w-]+)-to-([^-]+)-([\\w-]+)/)) {
                        ...
                        !nodes[source] || nodes[source].addEventListener(chsrc, async event => {
    ...
    document.querySelectorAll(`[data-use-template]`).forEach(element => {
        const name = element.dataset.useTemplate;
        document.querySelectorAll(`[data-template='${name}']`).forEach(template => {
    ...
    document.querySelectorAll(`[data-glass="${name}"]`).forEach(glass => {
        glass.querySelectorAll("*").forEach(element => {
            ...
            element.querySelectorAll(`datalist[data-selector]`).forEach(datalist => {
                element.querySelectorAll(datalist.dataset.selector).forEach(child => {
                    datalist.querySelectorAll("option").forEach(option => {
                        ...
                        if (option.dataset.value[0] === "@") {
                            const value = option.dataset.value.slice(1);
                            target[keys[0]](...value.split(";"));
                            return;    
                        }
                        target[keys[0]] = option.dataset.value;
                    });
                });
            });
        });
        glass.querySelectorAll(`[data-extend]`).forEach(element => {
            ...
            let wall = parent;
            while (wall && !wall.dataset.component) {
                wall = wall.parentElement;
            }
            let root = wall;
            while (root && !root.dataset.glass) {
